import { FestivalInfo } from '@/app/lib/types';

export const festivalLinupes: FestivalInfo[] = [
  {
    subId: '',
    name: 'Danfesta 2024',
    addr: '단국대 죽전캠퍼스',
    startDate: '2024-05-22',
    endDate: '2024-05-23',
    festivalStatus: 'BEFORE',
    description: '단국대학교 축제 Orbit',
    url: 'https://www.instagram.com/p/C6aPtvXBs16/?img_index=1',
    festivalFiles: [
      {
        originalFileName: 'image',
        mimeType: 'image/jpeg',
        file: 'https://scontent.cdninstagram.com/v/t39.30808-6/441254653_18323237617127956_1374219647061733025_n.jpg?stp=dst-jpg_e35&efg=eyJ2ZW5jb2RlX3RhZyI6ImltYWdlX3VybGdlbi4xNDQweDE0NDAuc2RyLmYzMDgwOCJ9&_nc_ht=scontent.cdninstagram.com&_nc_cat=104&_nc_ohc=iYQNJUVmoAEQ7kNvgHM1K4_&edm=APs17CUAAAAA&ccb=7-5&ig_cache_key=MzM1ODA2NTU3NTk5MzAwMTk2Mw%3D%3D.2-ccb7-5&oh=00_AYDAcqDcGs8oZCPIBhUSQvzy4OXSRHwIKKAPuH12o3IJNA&oe=664775DA&_nc_sid=10d13b',
        type: 'POSTER',
      },
    ],
  },
  {
    subId: '',
    name: '서강대학교',
    addr: '서강대학교',
    startDate: '2024-05-16',
    endDate: '2024-05-17',
    festivalStatus: 'BEFORE',
    description: '2024년 서강대 대동제: 오월 푸른달',
    url: 'https://sgyoonseul24.notion.site/2024-3e13ac386eea491ca3a37f1d260aeb01',
    festivalFiles: [
      {
        originalFileName: 'image',
        mimeType: 'pdf',
        file: 'https://sgyoonseul24.notion.site/image/https%3A%2F%2Fprod-files-secure.s3.us-west-2.amazonaws.com%2F6e4b63c2-b1b3-4c65-bddb-dcabd308584a%2Fb19bcb47-b721-4555-80cb-2883d71714d7%2F24_%25E1%2584%2583%25E1%2585%25A2%25E1%2584%2583%25E1%2585%25A9%25E1%2586%25BC%25E1%2584%258C%25E1%2585%25A6_%25E1%2584%2591%25E1%2585%25A9%25E1%2584%2589%25E1%2585%25B3%25E1%2584%2590%25E1%2585%25A5(%25E1%2584%2582%25E1%2585%25A9%25E1%2584%2589%25E1%2585%25A7%25E1%2586%25AB%25E1%2584%258B%25E1%2585%25AD%25E1%2586%25BC).jpg?table=block&id=7627f7aa-2689-4357-92f4-89432176290f&spaceId=6e4b63c2-b1b3-4c65-bddb-dcabd308584a&width=2000&userId=&cache=v2',
        type: 'POSTER',
      },
    ],
  },
  {
    subId: '',
    name: '백상대동제',
    addr: '동국대학교',
    startDate: '2024-05-28',
    endDate: '2024-05-30',
    festivalStatus: 'BEFORE',
    description: '봄 대동제 : 無我之境',
    url: 'https://www.instagram.com/donggukstuco/',
    festivalFiles: [
      {
        originalFileName: 'image',
        mimeType: 'image/jpeg',
        file: 'https://scontent.cdninstagram.com/v/t51.29350-15/434278821_1095433795006344_7895294653426061124_n.jpg?stp=dst-jpg_e15&efg=eyJ2ZW5jb2RlX3RhZyI6ImltYWdlX3VybGdlbi4xMDgweDEwODAuc2RyLmYyOTM1MCJ9&_nc_ht=scontent.cdninstagram.com&_nc_cat=100&_nc_ohc=8Df5XyVfcacQ7kNvgGJea8m&edm=APs17CUBAAAA&ccb=7-5&ig_cache_key=MzMyOTg2MDc4OTQ3NzI2NTcwOA%3D%3D.2-ccb7-5&oh=00_AYAt9ZHVgDqOb-dj7jRqse56Kb8nKzEtI7VJ8CE1YsSYnQ&oe=66479635&_nc_sid=10d13b',
        type: 'POSTER',
      },
    ],
  },
  {
    subId: '',
    name: '2024 LUCAUS',
    addr: '중앙대학교',
    startDate: '2024-05-27',
    endDate: '2024-05-30',
    festivalStatus: 'BEFORE',
    description: '청:용오름, 청의 중앙에서 비상하다',
    url: 'https://www.instagram.com/cau.festival/',
    festivalFiles: [
      {
        originalFileName: 'image',
        mimeType: 'image/jpeg',
        file: 'https://scontent.cdninstagram.com/v/t51.29350-15/437803630_991220128527557_3806237216357336290_n.jpg?stp=dst-jpg_e35&efg=eyJ2ZW5jb2RlX3RhZyI6ImltYWdlX3VybGdlbi4yMDAweDIwMDAuc2RyLmYyOTM1MCJ9&_nc_ht=scontent.cdninstagram.com&_nc_cat=105&_nc_ohc=qW31qO909K8Q7kNvgF1eN_Y&edm=APs17CUBAAAA&ccb=7-5&ig_cache_key=MzM0NDUyNzMxMTEzODk3Nzc3MA%3D%3D.2-ccb7-5&oh=00_AYBn7urmS-9p0Pz25zqYpuG_6CrTCl9VVAZaaM49jutDBA&oe=6645B64A&_nc_sid=10d13b',
        type: 'POSTER',
      },
    ],
  },
];
